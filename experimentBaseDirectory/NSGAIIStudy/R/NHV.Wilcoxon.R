...
### START OF SCRIPT 
# Constants
problemList <-c("scheduleProblem", "roomAllocationProblem") 
algorithmList <-c("NSGAIIa") 
tabularString <-c("l") 
...
# Step 3. Problem loop 
latexTableHeader("scheduleProblem roomAllocationProblem ", tabularString, latexTableFirstLine)
    ...
    for (j in algorithmList) {
      for (problem in problemList) {
        if (jndx != 0) {
          ...
          if (problem == "roomAllocationProblem") {
            if (j == "NSGAIIa") {
              write(" \\\\ ", "NHV.Wilcoxon.tex", append=TRUE)
            } 
            else {
